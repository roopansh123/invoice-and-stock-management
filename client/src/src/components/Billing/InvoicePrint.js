      ...
      <table style={tbl}>
        <tbody><tr>
          <td style={{ ...cell, textAlign: 'center', padding: '10px 6px' }}>
            ...
            <div style={{ fontSize: '10px' }}><strong>PAN : {co.pan}</strong></div>
      ...
      <table style={tbl}>
        <thead>
          <tr>
            ...
            <th style={{ ...th, textAlign: 'left', width: '32%' }}>Description of Goods</th>
            <th style={{ ...th, width: '10%' }}>HSN/SAC<br/>Code</th>
            <th style={{ ...th, width: '9%' }}>Qty.</th>
            <th style={{ ...th, width: '7%' }}>Unit</th>
            <th style={{ ...th, width: '11%' }}>Price</th>
        ...
        <tbody>
          {(invoice.items || []).map((item, i) => (
            <tr key={i}>
              ...
              <td style={{ ...cell, textAlign: 'center' }}>{item.quantity}.000</td>
              <td style={{ ...cell, textAlign: 'center' }}>PCS</td>
          ...
          {/* Subtotal */}
          <tr>
            <td colSpan={6} style={{ ...cell, textAlign: 'right', borderRight: '1px solid #000' }}></td>
          ...
          {isIntra && (
            <tr>
              <td colSpan={5} style={{ ...cell, textAlign: 'right' }}>Add : SGST</td>
          ...
          <tr style={{ background: '#f0f0f0' }}>
            ...
            <td colSpan={2} style={{ ...cell, textAlign: 'center', fontWeight: 'bold' }}>{totalQty}.000 Units</td>
            <td style={cell}></td>
        ...
        <tbody>
          {hsnSummary.map((row, i) => (
            <tr key={i}>
              ...
              {isIntra ? <><td style={{ ...cell, textAlign: 'right' }}>{fmt(row.cgst)}</td><td style={{ ...cell, textAlign: 'right' }}>{fmt(row.sgst)}</td></> : <td style={{ ...cell, textAlign: 'right' }}>{fmt(row.igst)}</td>}
          ...
          <tr style={{ fontWeight: 'bold', background: '#f8f8f8' }}>
            ...
            {isIntra ? <><td style={{ ...cell, textAlign: 'right' }}>{fmt(invoice.totalCGST)}</td><td style={{ ...cell, textAlign: 'right' }}>{fmt(invoice.totalSGST)}</td></> : <td style={{ ...cell, textAlign: 'right' }}>{fmt(invoice.totalIGST)}</td>}
