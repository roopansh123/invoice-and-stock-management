...
function toPcs(qty, unit, bags, bagQty) {
  const fromBags = (Number(bags) || 0) * (Number(bagQty) || 12);
  const fromQty  = unit === 'dzn' ? (Number(qty) || 0) * 12 : (Number(qty) || 0);
  return fromBags + fromQty;
}
...
export default function NewBill() {
  ...
  useEffect(() => {
    if (productSearch.length >= 2)
      API.get('/products', { params: { search: productSearch, limit: 10 } }).then(r => setProducts(r.data.products));
    else setProducts([]);
  }, [productSearch]);

  const calcItem = (item) => {
    const pcs     = toPcs(item.qty, item.unit, item.bags, item.bagQty);
    const gross   = item.unitPrice * pcs;
    ...
    return { ...item, quantity: pcs, taxableAmount: taxable, cgst, sgst, igst, total: taxable + gstAmt };
  };
  ...
  const addProduct = (product, size) => {
    ...
    if (existing >= 0) {
      const updated = [...items];
      updated[existing].qty = (Number(updated[existing].qty) || 0) + 1;
      setItems(updated);
    } else {
      setItems(prev => [...prev, {
        productId: product._id, sku: product.sku, name: product.name,
        size, bags: 0, bagQty: product.bagQty || 12, qty: 1, unit: 'pcs',
        isMixLot: product.isMixLot || false,
        unitPrice: product.sellingPrice, discount: 0, discountType: 'percent', gstRate: product.gstRate,
      }]);
    }
    setProductSearch(''); setProducts([]);
  };

  const addMixLot = (product) => addProduct(product, 'Mix Lot');

  const updateItem = (idx, field, val) => {
    setItems(prev => {
      const a = [...prev];
      a[idx] = { ...a[idx], [field]: ['bags','bagQty','qty','discount','unitPrice'].includes(field) ? Number(val) : val };
      return a;
    });
  };
  ...
  const handleCreateCustomer = async (e) => {
    ...
    try {
      ...
      setSelectedCustomer(res.data); setCustomerSearch(res.data.name);
      setShowNewCustomer(false); toast.success('Customer created!'); setStep(2);
  ...
  const handleSubmit = async () => {
    ...
    const hasEmpty = calcedItems.some(i => i.quantity <= 0);
    if (hasEmpty) { toast.error('All items must have qty > 0'); return; }
    setSubmitting(true);
    try {
      const payload = {
        customerId: selectedCustomer._id,
        items: calcedItems.map(i => ({
          product: i.productId, sku: i.sku, name: i.name, size: i.size,
          unit: i.unit, unitQty: i.qty || 0, bags: i.bags || 0, bagQty: i.bagQty,
          quantity: i.quantity, unitPrice: i.unitPrice,
          discount: i.discount, discountType: i.discountType, gstRate: i.gstRate,
        })),
  ...
  const inputStyle = { background:'var(--surface2)', border:'1px solid var(--border)', color:'var(--text)', padding:'4px', borderRadius:'4px', textAlign:'center', fontFamily:'var(--font-mono)' };
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:'20px',alignItems:'start'}}>
          ...
          {selectedCustomer&&(
            <div className="card" style={{marginBottom:'16px'}}>
              ...
              <div style={{position:'relative',marginBottom:'16px'}}>
                <input className="input" placeholder="Search products by name, SKU..." value={productSearch} onChange={e=>setProductSearch(e.target.value)}/>
                {products.length>0&&(
                    ...
                    {products.map(p=>(
                      <div key={p._id} style={{padding:'10px 14px',borderBottom:'1px solid var(--border)'}}>
                        ...
                        <div style={{display:'flex',gap:'6px',flexWrap:'wrap',alignItems:'center'}}>
                          {/* Mix Lot button — always shown */}
                          <button onClick={()=>addMixLot(p)}
                            style={{padding:'4px 10px',background:'rgba(167,139,250,0.15)',border:'1px solid rgba(167,139,250,0.5)',borderRadius:'var(--radius-sm)',color:'#a78bfa',cursor:'pointer',fontSize:'12px',fontWeight:700}}>
                            📦 Mix Lot ({p.totalStock})
                          </button>
                          {/* Size buttons */}
              ...
              {items.length>0 ? (
                <div style={{overflowX:'auto'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',minWidth:'780px'}}>
                    <thead>
                      <tr style={{background:'var(--surface2)'}}>
                        <th style={{textAlign:'left',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>PRODUCT</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>SIZE</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px',background:'rgba(234,179,8,0.1)'}}>BAGS</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px',background:'rgba(234,179,8,0.1)'}}>1BAG=PCS</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>QTY</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>UNIT</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--green)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>TOTAL PCS</th>
                        <th style={{textAlign:'right',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>PRICE/PCS</th>
                        <th style={{textAlign:'center',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>DISC%</th>
                        <th style={{textAlign:'right',fontSize:'11px',color:'var(--muted)',padding:'8px',fontFamily:'var(--font-mono)',letterSpacing:'1px'}}>TOTAL</th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                      {calcedItems.map((item,i)=>{
                        ...
                        return (
                          <tr key={i} style={{borderTop:'1px solid var(--border)'}}>
                            ...
                            <td style={{padding:'8px',textAlign:'center',fontFamily:'var(--font-mono)',fontSize:'13px',fontWeight:600}}>{item.size}</td>

                            {/* BAGS column */}
                            <td style={{padding:'8px',textAlign:'center',background:'rgba(234,179,8,0.05)'}}>
                              <input type="number" min="0" value={item.bags || ''} placeholder="0"
                                onChange={e=>updateItem(i,'bags',e.target.value)}
                                style={{...inputStyle, width:'52px', borderColor:'rgba(234,179,8,0.4)'}}/>
                            </td>

                            {/* 1 BAG = PCS */}
                            <td style={{padding:'8px',textAlign:'center',background:'rgba(234,179,8,0.05)'}}>
                              <input type="number" min="1" value={item.bagQty}
                                onChange={e=>updateItem(i,'bagQty',e.target.value)}
                                style={{...inputStyle, width:'52px', borderColor:'rgba(234,179,8,0.4)', color:'var(--gold)', fontWeight:700}}/>
                            </td>
                            ...
                            <td style={{padding:'8px',textAlign:'center'}}>
                              <select value={item.unit} onChange={e=>updateItem(i,'unit',e.target.value)}
                                style={{background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--text)',padding:'4px 6px',borderRadius:'4px',fontSize:'12px',fontFamily:'var(--font-mono)',cursor:'pointer'}}>
                            ...
                            {/* TOTAL PCS (read only) */}
                            <td style={{padding:'8px',textAlign:'center'}}>
                              <span style={{fontSize:'14px',fontWeight:800,fontFamily:'var(--font-mono)',color:'var(--green)'}}>
                                {pcs}
                              </span>
                            </td>
                            ...
                            <td style={{padding:'4px'}}>
                              <button onClick={()=>removeItem(i)} style={{background:'none',border:'none',color:'var(--red)',cursor:'pointer',fontSize:'16px',padding:'4px'}}>✕</button>
                            </td>
                          </tr>
                        );
                      })}
                  ...
                  {/* Footer summary */}
                  <div style={{marginTop:'10px',padding:'8px 12px',background:'var(--surface2)',borderRadius:'6px',fontSize:'12px',color:'var(--muted)',display:'flex',gap:'20px',flexWrap:'wrap'}}>
                    <span>Items: <strong style={{color:'var(--text)'}}>{items.length}</strong></span>
                    <span>Total pcs: <strong style={{color:'var(--green)'}}>{calcedItems.reduce((s,i)=>s+i.quantity,0)}</strong></span>
                    {calcedItems.some(i=>i.bags>0) && <span>Bags: <strong style={{color:'var(--gold)'}}>{calcedItems.reduce((s,i)=>s+(Number(i.bags)||0),0)}</strong></span>}
                    {calcedItems.some(i=>i.unit==='dzn'&&i.qty>0) && <span>Dzn qty: <strong style={{color:'var(--text)'}}>{calcedItems.filter(i=>i.unit==='dzn').reduce((s,i)=>s+(Number(i.qty)||0),0)}</strong></span>}
                  </div>
                </div>
              ) : (
          ...
          {selectedCustomer&&items.length>0&&(
            <div className="card">
              ...
              <div className="input-row cols-3">
                <div className="input-group"><label className="input-label">Invoice Type</label>
                  <select className="input" value={invoiceType} onChange={e=>setInvoiceType(e.target.value)}>
                    <option value="GST">GST Invoice</option><option value="Non-GST">Non-GST</option><option value="Cash">Cash Bill</option>
                  </select>
                </div>
                <div className="input-group"><label className="input-label">Sale Type</label>
        ...
        <div style={{position:'sticky',top:'0'}}>
          <div className="card">
            <div style={{fontWeight:700,fontSize:'15px',marginBottom:'16px',borderBottom:'1px solid var(--border)',paddingBottom:'12px'}}>💳 Invoice Summary</div>
            {items.length===0 ? (
              ...
              <>
                ...
                <div className="input-group">
                  ...
                  <input className="input" type="number" value={paidAmount} onChange={e=>setPaidAmount(e.target.value)} placeholder={grandTotal} min="0" max={grandTotal}/>
                </div>
                {paidAmount!==''&&(
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                  <button className="btn btn-primary" onClick={handleSubmit} disabled={submitting||!selectedCustomer||items.length===0} style={{justifyContent:'center',padding:'12px'}}>
                    {submitting?'Creating...':'🧾 Create Invoice'}
                  </button>
                  <div style={{fontSize:'11px',color:'var(--muted)',textAlign:'center'}}>
                    {items.length} item{items.length!==1?'s':''} · {calcedItems.reduce((s,i)=>s+i.quantity,0)} pcs total
                  </div>
                </div>
              </>
            )}
